...
trait Tuple2OpsModule {
  ...
  private def render(
    ...
    val renderedAndWrappedKey =
      renderedKey.wrappedOnSpaces(availableWidthInCharacters)
    val renderedAndWrappedValue =
      renderedValue.wrappedOnSpaces(availableWidthInCharacters)

    val separator = " -> "
    val leftPadding = "  "

    lazy val simpleRendering =
      s"$renderedAndWrappedKey$separator$renderedAndWrappedValue"

    lazy val complexRendering =
      (renderedAndWrappedKey + separator).wrappedOnSpaces(
        availableWidthInCharacters
      ) + "\n" +
        (renderedValue
          .wrappedOnSpaces(availableWidthInCharacters - leftPadding.size))
          .split("\n")
          .toList
          .map(leftPadding + _)
          .mkString("\n")
