...
class StreamOpsTests extends spells.UnitTestConfiguration {
  test(
    "The arguments to the Stream factory from spells should be evaluated only on demand"
  ) {
  ...
  test(
    "The Stream produced by the factory from spells should equal the Stream produced by the standard library"
  ) {
    MakeStream(1, 2) should be(scala.collection.immutable.Stream(1, 2))
  }

  test(
    "The Stream factory from spells should not shadow Stream's companion object"
  ) {
  ...
  test(
    ...
    def good[E](argument: spells.StreamOpsModule.Function0Wrapper[E]): Any =
      argument
    good(1)
